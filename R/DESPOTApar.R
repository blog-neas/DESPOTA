...
#'
#' @return a list 
#' @import dendextend parallel fpc fastcluster
...
despotaPar <- function(data, distmat = NULL, distMethod = "euclidean", 
  ...
  stopifnot(is.null(ncores) | (ncores > 0 & ncores <= parallel::detectCores(logical = FALSE)))
 ...
 # suppressMessages(library(dendextend))
  ...
  # LOOP PARALLELIZZATO! ----------------------
  # -------------------------------------------
  permutation <- function(kkk){
    nr_reps <- ceiling(M/ncores)
    ripeti <- nr_reps
    rcm <- rep(NA,ripeti)
    for(kk in 1:ripeti){
      ...
      nk <- length(obsk)
      splij <- split(obsij, cutree(hcij, 2)) # WOT?!?!?!?!
      ni <- length(splij[[1]])
      nj <- length(splij[[2]])
      ...
      heiN <- switch(agglMethod,
                     ward.D = ((ni + nk) * heightik + (nj + nk) * heightjk - nk * heightij)/(ni + nj + nk),
                     ward.D2 = sqrt(((ni + nk) * heightik^2 + (nj + nk) * heightjk^2 - nk * heightij^2)/(ni + nj + nk)),
      ...
      if(!(lh == 0 & rh == 0) | MAT$membLeft[i] > 1 | MAT$membRight[i] > 1) {
        ...
        while(start <= M){
          ...
          parallel::clusterExport(cl = cl, varlist = c(
            "i", "M", "ncores",
            "MAT", "lh", "rh", "lobs", "robs",
            "distMat", "ClusterRight", "ClusterLeft"
          ),
          envir = environment()
          )
          # nr_reps <- ceiling(M/ncores)
          rcm <- parallel::clusterApply(cl = cl, 1:ncores, fun = permutation )
          rcm <- unlist(rcm)
          rcm <- rcm[1:M]
          # 
          # rcm <- parallel::mclapply(start:M, FUN = permutation,
          #                           i = i,
          #                           MAT=MAT, lh=lh, rh=rh, lobs=lobs, robs=robs,
          #                           distMat=distMat, ClusterRight=ClusterRight, ClusterLeft=ClusterLeft
          # )
          # rcm <- unlist(rcm)
          # --------------------------------------
